...
class FluentController extends Controller
{
    ...
    {
        $users = DB::table('Operateri')->select(['id', 'name', 'email', 'created_at', 'updated_at']);
    ...
    {
        $first = DB::table('Operateri')
            ->select(['id', 'name', 'email', 'created_at', 'updated_at'])
            ->where('name', 'like', '%jon%');

        $users = DB::table('Operateri')
            ->select(['id', 'name', 'email', 'created_at', 'updated_at'])
            ->where('name', 'like', '%sus%')
            ->union($first);
    ...
    {
        $posts = DB::table('posts')->join('Operateri', 'posts.user_id', '=', 'Operateri.id')
    ...
    {
        $users = DB::table('Operateri')->select([
        ...
        if ($name = $request->get('name')) {
            $datatables->where('Operateri.name', 'like', "$name%"); // additional Operateri.name search
        }

        // Global search function
        if ($keyword = $request->get('search')['value']) {
            // override Operateri.name global search
            $datatables->filterColumn('Operateri.name', 'where', 'like', "$keyword%");

            // override Operateri.id global search - demo for concat
            $datatables->filterColumn('Operateri.id', 'whereRaw', "CONCAT(Operateri.id,'-',Operateri.id) like ? ", ["%$keyword%"]);
        }
