...
;(function ($, document, window) {
	...
	$.fn.userTimeout = function (opts) {
		...
		var defaults = {
			...
			timer: true,
			session: 600000,                   // 10 Minutes in Milliseconds, then notification of logout // 600000
			force: 30000,                     // 30 Sekundi, then logout
			ui: 'bootstrap',                        // Model Dialog selector (auto, bootstrap, jqueryui)
			debug: true,					   // Shows alerts
			modalTitle: 'Istek sesije',     // Modal Title
			modalBody: 'Dogodio se time-out zbog neaktivnosti. Molimo odaberite da li želite nastaviti raditi ili se želite odjaviti. inače biti čete odjavljeni automatski.'  // Modal Body
		};
		...
		var modal = function () {
			...
			if (modalUI === 'bootstrap') {
				...
				container = $('<div class="modal fade" id="notifyLogout" class="modal fade" role="dialog" data-modal-index="3" role="dialog" aria-labelledby="notifyLogoutLabel" aria-hidden="true"></div>');
